...
#
# [개요]
# 7개 소스(네이버 경제/증권·4개 금융 RSS·KRX 공시)에서 최근 2일치 뉴스를
...
#    - 네이버 경제 헤드라인 (sid1=101) : 2일 × 15건
#    - 네이버 증권 섹션   (sid2=258)  : 2일 × 10건
#    - 한국경제 RSS                   : 최대 20건
#    - 연합뉴스 경제 RSS              : 최대 20건
#    - 매일경제 RSS                   : 최대 20건
#    - 서울경제 RSS                   : 최대 20건
...
def get_naver_securities_news(days=2, limit_per_day=10, seen_urls=None):
    """네이버 뉴스 증권 섹션(sid2=258) 수집 — 종목/수급/실적 특화"""
...
def collect_all_news():
    """7개 소스 병렬 수집 후 URL 기준 중복 제거 (네이버경제·증권 + RSS 4곳 + KRX공시)."""
    def _economy():
...
# ── 병렬 분석용 세마포어 ─────────────────────────────────────────────────────
# gemini-2.5-flash RPM 2000 기준으로 5건 동시 호출은 충분히 안전한 수준
...
규칙:
- 투자관련 true: 주가·실적·정책·산업·M&A·공시 등 / false: 소비자·연예·스포츠 등
- 종목관련: 특정 종목 직접 언급 또는 수혜 추론 가능 / 시장일반: 금리·환율·거시경제
- 직접수혜종목: 이 뉴스가 호재인 상장사만 (악재 종목 절대 제외)
- 간접수혜종목: 공급망·경쟁구도·정책 수혜 연관 상장사 최소 2개 추론
- 공급망연관종목: 직접수혜종목의 납품업체·고객사 중 상장사
- 부정영향종목: 이 뉴스로 악재를 받을 상장사
- 폭등전조유형(해당 시): 제도정책빅뱅/독점최초계약/실적서프라이즈/대규모지분변동/신사업기술돌파/수급변화/M&A합병
...
{{
    ...
    "가치주신호": true/false,
    "업종": "",
    "투자호재성격": "제도정책/독점최초/실적서프라이즈/지분변동/신사업기술/수급변화/M&A/기타",
    "폭등전조유형": "",
    "단기폭발력": 1~10,
    "매출증가율": null,
    "영업익증가율": null,
    ...
    response = _gemini_generate(prompt, model=GEMINI_MODEL_LITE, thinking=False)
...
아래는 최근 2일간 경제·증권 뉴스 {len(results)}건의 AI 분석 결과입니다.
...
def _send_email(output_file: str, stocks_all: list, df_watch, df_caution) -> None:
    ...
    body = "\n".join([
        ...
        *(caution_lines if caution_lines else ["  (악재 경고 없음)"]),
        ...
        f"",
        f"■ 동작 방식",
        f"  최근 2일치 뉴스 중복 제거 후 약 100건 수집 (7개 소스 병렬)",
...
if __name__ == "__main__":
    ...
    # ── 2단계: 기사 본문 병렬 prefetch ────────────────────────────────────────
    console.print("[bold]▶ 기사 본문 병렬 로딩 중...[/bold]")
    with Progress(
        SpinnerColumn(),
        TextColumn("{task.description}", table_column=Column(width=20, no_wrap=True)),
        BarColumn(),
        ...
        t_fetch = p.add_task("본문 로딩...", total=len(raw_news))
        prefetched: list[str] = [""] * len(raw_news)
        with ThreadPoolExecutor(max_workers=10) as ex:
            fmap = {ex.submit(_fetch_content, n['link']): i for i, n in enumerate(raw_news)}
            for fut in as_completed(fmap):
                prefetched[fmap[fut]] = fut.result() or ""
                p.advance(t_fetch)
    console.print(f"[green]✔[/green] 본문 로딩 완료 ({sum(1 for c in prefetched if c)}건 성공)\n")
    ...
    console.print(f"[bold]▶ 개별 기사 AI 분석 중... ({len(pre_filtered)}건, 병렬 {GEMINI_CONCURRENCY}건)[/bold]")
    final_insights = []
    failed         = []
    _lock          = threading.Lock()
    completed      = [0]  # mutable counter for thread-safe increment

    def _analyze_task(idx, news):
        """Thread worker: 세마포어로 동시 호출 수 제어."""
        with _gemini_sem:
            try:
                analysis = _analyze_one(news, content=prefetched[idx])
                with _lock:
    ...
    with Progress(
        ...
        t_analyze = progress.add_task("AI 병렬 분석 중...", total=total)

        with ThreadPoolExecutor(max_workers=GEMINI_CONCURRENCY) as ex:
            futures = {ex.submit(_analyze_task, idx, news): news for idx, news in pre_filtered}
            for fut in as_completed(futures):
                fut.result()  # 예외는 _analyze_task 내부에서 처리됨
                done = completed[0]
                progress.update(t_analyze, completed=done,
                                description=f"[{done}/{total}] 병렬 분석 중...")
    ...
    output_file = os.path.join(output_dir, f"뉴스기사_인사이트_{timestamp}.xlsx")
