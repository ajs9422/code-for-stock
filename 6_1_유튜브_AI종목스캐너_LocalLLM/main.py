...
def _ollama_generate(prompt: str, max_retries: int = 3) -> str | None:
...
def _llm_generate(prompt: str) -> _OllamaResponse:
    ...
    resp = gemini.models.generate_content(model="gemini-2.5-flash-lite", contents=prompt)
    return _OllamaResponse(resp.text)
...
def analyze_with_gemini(title, transcript, max_retries=3):
    ...
    prompt = f"""당신은 한국 주식 투자 전문가입니다. 아래 유튜브 영상 자막을 분석하세요.
...
{{
  ...
  "요약": "핵심 내용 3줄 이내",
  "추천종목": [
    {{"종목명": "삼성전자", "사유": "추천 이유를 자막 내용 기반으로 작성"}},
    {{"종목명": "SK하이닉스", "사유": "추천 이유를 자막 내용 기반으로 작성"}}
  ]
}}

[규칙]
1. 투자관련: 주식·ETF·펀드·기업·산업·경제 관련이면 true, 연예·스포츠·날씨만 다루면 false
2. 출연자: 자막에서 파악된 실명만 쉼표 구분, 모르면 빈 문자열
3. 추천종목: 자막에서 언급된 한국 증권시장 상장 종목을 모두 추출하세요
   - "사야 한다", "좋다", "오른다", "주목", "관심", "매수" 등 긍정 맥락이면 포함
   - 삼성전자, SK하이닉스, 현대차 같은 종목명이 나오면 반드시 포함
   - 비상장 기업, 해외 주식, ETF 이름이 구체적으로 언급된 경우도 포함
   - 종목 언급이 전혀 없을 때만 빈 배열 []
4. 종목명은 한국 주식시장에 실제 상장된 정확한 회사명으로 작성"""
    for attempt in range(max_retries):
        try:
            response = _llm_generate(prompt)
            result = _parse_gemini_response(response.text)
            # 추천종목이 비어있으면 1회 재시도 (Local LLM 보수적 판단 보완)
            if USE_LOCAL_LLM and not result.get("추천종목") and result.get("투자관련"):
                console.print(f"  [yellow][추천종목 재시도][/yellow] {title[:30]}")
                response2 = _llm_generate(prompt)
                result2 = _parse_gemini_response(response2.text)
                if result2.get("추천종목"):
                    return result2
            return result
        except Exception as e:
            ...
            if "429" in err_str:
                ...
                console.print(f"[red]  [ERR] LLM 분석 실패: {e}[/red]")
                return None

            if attempt < max_retries - 1:
                console.print(f"[yellow]  >> LLM 일시 오류, {wait}초 대기 후 재시도 ({attempt+1}/{max_retries})...[/yellow]")
                time.sleep(wait)
            else:
                console.print(f"[red]  [ERR] LLM 재시도 {max_retries}회 실패: {e}[/red]")
                return None
...
console.print()
console.print("════════════════════════════════════════════════════════════════")
console.print("  [bold cyan]유튜브 AI 종목스캐너[/bold cyan] — 채널 추천 종목 분석")
console.print("════════════════════════════════════════════════════════════════")
if USE_LOCAL_LLM:
    ...
    console.print(f"  [dim]폴백[/dim]: {fallback_note.strip()}")
    if not _check_ollama():
        console.print("[red]Ollama를 먼저 실행하세요: ollama serve[/red]")
        sys.exit(1)
else:
    console.print(f"  [cyan]AI 엔진[/cyan]: Gemini (gemini-2.5-flash-lite)")
console.print()

# 종목맵 빌드를 채널 수집과 병렬 실행
...
with Progress(
    ...
    # 종목맵 준비 완료 (채널 수집 중 병렬 빌드)
    STOCK_RAW_MAP, STOCK_NORM_MAP = _stock_map_fut.result()
    ...
    for name, v in all_videos:
        ...
        if transcript is not None:
            if not USE_LOCAL_LLM:
                time.sleep(1)   # gemini-2.5-flash-lite 유료 속도 제한
...
suffix      = "_LocalLLM" if USE_LOCAL_LLM else ""
output_file = os.path.join(output_dir, f"유튜브_AI종목스캐너_{timestamp}{suffix}.xlsx")
